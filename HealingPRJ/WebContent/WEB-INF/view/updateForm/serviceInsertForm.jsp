...
<%
	...
	List<ServiceInsertDTO> sList = (List<ServiceInsertDTO>)request.getAttribute("serviceList");
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">서비스환경 만족도 수정</h1>
			</div>
		</div>
		<!--/.row-->
				...
				<div class="panel panel-default chat">
						...
						<div style="height: auto; width: auto;">
							<div style="display: inline-block;">
								...
								<div style="width: 120px; float: left; margin-right: 10px; white-space: nowrap; max-width: 300px;">
									<input class="form-control form-agency" name="form-agency" placeholder="기관명" value="<%=convertXSS(sList.get(0).getAgency())%>">
								</div>
								
								<h4 class="item-title" style="float: left; margin: 8px 5px 8px 0px;">시작일자</h4> 
								<div style="float: left; margin-right: 10px;">
									<input class="form-control form-openday" style="width: 190px;" type="date" name="form-openday" value="<%=convertXSS(sList.get(0).getOpenday())%>">
								</div>
							</div>
							...
							<div style="display: block;">
								...
								<div style="margin-right:10px">
									...
									<div style="float: left; margin-right: 10px;">
										<select class="form-control" name="form-ptcProgram" style="width: 120px; height: 35px;">
											<option><%=convertXSS(sList.get(0).getPtcProgram())%></option>
											<option>당일형</option>
											<option>1박2일형</option>
											<option>2박3일형</option>
										</select>
								...
								<div style="margin-right:10px">
									...
									<div style="float: left; margin-right: 10px;">
										<input class="form-control form-eval_date" style="width: 190px;" type="date" name="form-eval_date" value="<%=convertXSS(sList.get(0).getEval_date())%>">
									</div>
								</div>
					...
					<div class="panel-body" style="overflow-x: scroll; height: auto; overflow-y: hidden;">
						...
						<div style="width: 2400px; height: 70px">
							<div style="width: 85px; float: left; margin-right: 10px; margin-left:30px; text-align: center; -webkit-text-emphasis-style: open;">
							...
							<div class="insertForm" style="width: 2400px; height: 40px" name="form-main" id="child<%=i%>">
								...
								<div style="width: 85px; float: left; margin-right: 10px;">
									<select class="form-control" name="form-sex" style="height: 35px;">
										<option><%=convertXSS(sList.get(i).getSex())%></option>
										<%@ include file="/WEB-INF/view/insertForm/sex.jsp"%>
									</select>
								</div>
								<div style="width: 65px; float: left; margin-right: 10px;">
									<input class="form-control" type="number" name="form-age" placeholder="연령" value="<%=convertXSS(sList.get(i).getAge())%>">
								</div>
								<div style="width: 90px; float: left; margin-right: 10px;">
									<select class="form-control" name="form-residence"
										style="height: 35px;">
										<option><%=convertXSS(sList.get(i).getResidence())%></option>
								...
								<div style="width: 140px; float: left; margin-right: 10px;">
									<select class="form-control" name="form-job" style="height: 35px;">
										<option><%=convertXSS(sList.get(i).getJob())%></option>
								...
								<div style="width: <%=groupWidth[j] %>px; float: left; margin-right: <%=groupMargin[j] %>px;">
									<input class="form-control" name="form-<%=form_name[j] %>" placeholder="<%=form_placeholder[j] %>" value="<%=sList.get(i).getScoreString2List().get(j)%>">
								</div>
								<%} %>
							</div>
							<%} %>
	...
	<script type="text/javascript">
		<%for (int i = 0; i < sList.size(); i++) {%>
	 	  ...
	 	  $('select.form-residence option[value=' + <%=convertXSS(sList.get(0).getResidence())%> + ']').attr('selected', 'selected');
	 	});
	 	<%}%>
		function add_div() {
	    ...
	    function action() {
	      ...
	      for(let i = 0; i < formArr.length; i++) {
	         ...
	         let totalLength = age.length + score1.length + score2.length + score3.length + score4.length 
				+ score5.length + score6.length + score7.length + score8.length + score9.length + score10.length + score11.length
				+ score12.length + score13.length + score14.length + score15.length + score16.length + score17.length + score18.length;
