...
<body onload="returnState()">
	...
	<script type="text/javascript">
		...
		function testA(type, value, inx) {
			...
			switch (true) {
				case (val>0 && val<=70):
					showState(valText[inx], "#ff3d1f", "매우나쁨");
					break;
				case (val>70 && val<=90):
					showState(valText[inx], "#ff9e36", "나쁨");
					break;
				case (val>90 && val<=110):
					showState(valText[inx], "#21de33", "정상");
					break;
				case (val>110 && val<=130):
					showState(valText[inx], "#24cdf2", "좋음");
					break;
				case (val>130):
					showState(valText[inx], "#24cdf2", "매우좋음");
					break;
				default:
					break;
			}
		...
		function testB(type, value, inx) {
			...
			switch (true) {
			case (val>0 && val<=50):
				showState(valText[inx], "#24cdf2", "균형");
				break;
			case (val>50 && val<=100):
				showState(valText[inx], "#ff9e36", "불균형");
				break;
			case (val>100):
				showState(valText[inx], "#ff3d1f", "매우불균형");
				break;
			default:
				break;
			}
		...
		function testF(type, value, inx) {
			...
			switch (true) {
			case (val>0 && val<=60):
				showState(valText[inx], "#ff3d1f", "매우낮음");
				break;
			case (val>60 && val<=65):
				showState(valText[inx], "#ff9e36", "낮음");
				break;
			case (val>65 && val<=80):
				showState(valText[inx], "#21de33", "정상");
				break;
			case (val>80 && val<=100):
				showState(valText[inx], "#ff9e36", "높음");
				valText[inx].style.color = "#ff9e36";
				break;
			case (val>100):
				showState(valText[inx], "#ff3d1f", "매우높음");
				break;
			default:
				break;
			}
		...
		function testG(type, value, inx) {
			...
			case (val>90 && val<=110):
				...
				break;
			case (val>110 && val<=130):
				showState(valText[inx], "#24cdf2", "정상");
				break;
			case (val>130):
				showState(valText[inx], "#24cdf2", "매우좋음");
				break;
		...
		function action() {
			...
			for(let i = 0; i < formArr.length; i++) {
				...
				if(name.trim() == "" && id.trim() == "") {
					...
					if(num8.trim() == "") {
						doSwal("수치를 입력해주세요.", "", "info");
						return focusByName("form-num8", i);
					}
				}
				if(name.trim() == "" && id.trim() == "") {
					break;
