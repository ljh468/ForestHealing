...
<body>
	...
	<div class="insertForm rowdata" style="width: 2250px; height: 40px;" name="form-main" id="child<%=i%>">
		...
		<div style="width: 85px; float: left; margin-right: 10px;">
			<select class="form-control" name="form-sex" style="height: 35px;">
				<option><%=nvl(hList.get(i).getSex()) %></option>
				<%@ include file="/WEB-INF/view/insertForm/sex.jsp"%>
			</select>
		</div>
		<div style="width: 65px; float: left; margin-right: 10px;">
			<input class="form-control" type="number" name="form-age" placeholder="연령" value="<%=nvl(hList.get(i).getAge()) %>">
		</div>
		...
		<div style="width: 160px; float: left; margin-right: 5px;">
			<input class="form-control" type="number" name="form-pastStress" placeholder="0=미기재/1=무/2=유" value="<%=nvl(hList.get(i).getPast_stress_experience()) %>">
		</div>
		<%
